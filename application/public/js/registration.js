document.addEventListener('DOMContentLoaded', function() {
    ...
    function showError( message) {
        ...
        const existingError = Array.from(errorDiv.children).find(child => child.textContent === message);
    ...
    function validateUsername() {
        ...
        if (!usernameRegex.test(username)) {
            showError('Username must start with a letter and be at least 3 alphanumeric characters long.');
            return false;
        }
        return true;
    }
    ...
    function validatePassword() {
        ...
        if (!passwordRegex.test(password)) {
            showError('Password must be at least 8 characters long, contain 1 uppercase letter, 1 number, and 1 special character (/ * - + ! @ # $ ^ & ~ [ ]).');
            return false;
        }
        return true;
    }
